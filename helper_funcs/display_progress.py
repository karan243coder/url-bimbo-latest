...
async def _send_anime_sticker(client, chat_id, reply_to_msg_id=None):
    """Send a random anime sticker from Ufhjbdsvb folder. Returns message or None."""
    try:
        import glob
        import random
        import os
        
        # Find Ufhjbdsvb folder
        ...
        # Get random webp file
        files = [f for f in os.listdir(sticker_dir) if f.endswith('.webp') and not f.startswith('.')]
        if not files:
            return None
        
        sticker_path = os.path.join(sticker_dir, random.choice(files))
        ...
        logger.debug(f"Sent anime sticker: {sticker_path}")
        return sent
...
async def update_user_progress(client, user_id, force=False):
    ...
    async with _progress_lock(user_id):
        ...
        if sticker_msg is None and not _dashboard_is_photo.get(user_id, False):
            ...
            try:
                sticker_msg = await _send_anime_sticker(client, user_id, reply_to_msg_id=message.id)
                if sticker_msg:
