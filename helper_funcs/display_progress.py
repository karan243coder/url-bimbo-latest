...
def _get_random_header_image():
    """Get a random header image from admin panel settings."""
    try:
        from plugins.admin_panel import get_random_progress_image
        return get_random_progress_image()
    except Exception as e:
        logger.debug(f"Failed to get header image from admin: {e}")
        # Fallback to config
        try:
            from config import Config
            pic_config = getattr(Config, 'BIMBO_PROGRESS_PIC', '')
            if pic_config:
                import random
                images = [img.strip() for img in pic_config.split(',') if img.strip()]
                if images:
